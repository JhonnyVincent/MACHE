...
export async function registerVendorAction(formData: FormData) {
  /*
    ...
    Une exception ici rendrait « Application error » : la personne
    ne saurait pas si son compte a été créé, ne pourrait rien
    corriger, et n'aurait aucune raison de réessayer. On rend donc
    le formulaire avec une phrase, et la raison technique part au
    journal du serveur.
  ...
  */
  try {
    ...
    */
    ...
    reportOutage('inscription vendeur', reasonOf(error));

    redirect(
      `${BASE}?error=${encodeURIComponent(
        "L'ouverture de boutique n'a pas abouti. Réessayez dans quelques minutes ; si cela persiste, écrivez-nous."
      )}`
    );
  }
}
